...
namespace ProtoBuf.Reflection
{
    ...
    public class CSharpCodeGenerator : CommonCodeGenerator
    {
        ...
        protected override void WriteFileHeader(GeneratorContext ctx, FileDescriptorProto file, ref object state)
        {
            ...
            var tw = ctx.WriteLine("// This file was generated by a tool; you should avoid making direct changes.")
        ...
        protected override void WriteMessageHeader(GeneratorContext ctx, DescriptorProto obj, ref object state)
        {
            ...
            tw.WriteLine(")]");
            WriteOptions(ctx, obj.Options);
            tw = ctx.Write($"{GetAccess(GetAccess(obj))} partial class {Escape(name)} : AssemblyCommon.IProtoMessage");
        ...
        protected override void WriteField(GeneratorContext ctx, FieldDescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            ...
            tw.WriteLine(")]");
            //LiYu 去掉默认值IOS DefaultValue 会被裁剪
            ...
            if (isRepeated)
            {
                ...
                if (mapMsgType != null)
                {
                    ...
                    if (ctx.Supports(CSharp6))
                    {
                        ctx.WriteLine($"{GetAccess(GetAccess(obj))} global::System.Collections.Generic.Dictionary<{keyTypeName}, {valueTypeName}> {Escape(name)} {{ get; private set; }} = new global::System.Collections.Generic.Dictionary<{keyTypeName}, {valueTypeName}>();");
                    }
                    else
                    {
                ...
                {
                    ctx.WriteLine($"{GetAccess(GetAccess(obj))} global::System.Collections.Generic.List<{typeName}> {Escape(name)} {{ get; private set; }} = new global::System.Collections.Generic.List<{typeName}>();");
                }
                else
                {
        ...
        protected override void WriteEncodeBegin(GeneratorContext ctx, DescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            ctx.WriteLine("public void Encode(Google.Protobuf.CodedOutputStream writer)");
            ctx.WriteLine("{");
            ctx.Indent();
        }
        protected override void WriteEncodeBody(GeneratorContext ctx, FieldDescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            ...
            var tag = MakeTag(number, isMap, obj.type, obj.label);
            if (isRepeated)
            {
                if (isMap)
                {
                    var mapMsgType = ctx.TryFind<DescriptorProto>(obj.TypeName);
                    var keyType = mapMsgType.Fields.Single(x => x.Number == 1);
                    var valueType = mapMsgType.Fields.Single(x => x.Number == 2);

                    var ktag = MakeTag(1, false, keyType.type, keyType.label);
                    var vtag = MakeTag(2, false, valueType.type, valueType.label);
                    ctx.WriteLine($"foreach(var itor in {Escape(name)})");
                    ctx.WriteLine("{");
                    ctx.Indent();

                    ctx.WriteLine($"writer.WriteTag({tag});");
                    ctx.WriteLine("writer.WriteMapItor((mapw) => {");
                    ctx.Indent();
                    ctx.WriteLine($@"if(itor.Key != {DefaultValue(keyType.type)})");
                    ctx.WriteLine("{");
                    ctx.Indent();
                    ctx.WriteLine($"mapw.WriteTag({ktag});");
                    MakeEncode(ctx, keyType, "itor.Key", "mapw", "mapmstw");
                    ctx.Outdent();
                    ctx.WriteLine("}");
                    ctx.WriteLine($@"if(itor.Value != {DefaultValue(valueType.type)})");
                    ctx.WriteLine("{");
                    ctx.Indent();
                    ctx.WriteLine($"mapw.WriteTag({vtag});");
                    MakeEncode(ctx, valueType, "itor.Value", "mapw", "mapmstw");
                    ctx.Outdent();
                    ctx.WriteLine("}");
                    ctx.Outdent();
                    ctx.WriteLine("});");
                ...
                {
                    if(IsLengthDelimited(obj.type))
                    {
                        ctx.WriteLine($"foreach(var itor in {Escape(name)})");
                        ctx.WriteLine("{");
                        ctx.Indent();
                        ctx.WriteLine($"writer.WriteTag({tag});");
                        MakeEncode(ctx, obj, "itor", "writer", "msrw");
                        ctx.Outdent();
                        ctx.WriteLine("}");
                    }
                    else
                    {
                        ctx.WriteLine($"writer.WriteTag({tag});");
                        ctx.WriteLine("writer.WriteRepeated((listw) => {");
                        ctx.Indent();
                        ctx.WriteLine($"foreach(var itor in {Escape(name)})");
                        ctx.WriteLine("{");
                        ctx.Indent();
                        MakeEncode(ctx, obj, "itor", "listw", "listmsrw");
                        ctx.Outdent();
                        ctx.WriteLine("}");
                        ctx.Outdent();
                        ctx.WriteLine("});");
            ...
            {
                ctx.WriteLine($@"if({Escape(name)} != {DefaultValue(obj.type)})");
                ctx.WriteLine("{");
                ctx.Indent();
                ctx.WriteLine($"writer.WriteTag({tag});");
                MakeEncode(ctx, obj, Escape(name), "writer", "msrw");
                ctx.Outdent();
                ctx.WriteLine("}");
            }
        }
        static Dictionary<FieldDescriptorProto.Type, WireType> FieldWireMap = new Dictionary<FieldDescriptorProto.Type, WireType>() {
            {FieldDescriptorProto.Type.TypeDouble     , WireType.Fixed64},
            {FieldDescriptorProto.Type.TypeFloat      , WireType.Fixed32},
            {FieldDescriptorProto.Type.TypeInt64      , WireType.Variant},
            {FieldDescriptorProto.Type.TypeUint64     , WireType.Variant},
            {FieldDescriptorProto.Type.TypeInt32      , WireType.Variant},
            {FieldDescriptorProto.Type.TypeFixed64    , WireType.Fixed64},
            {FieldDescriptorProto.Type.TypeFixed32    , WireType.Fixed32},
            {FieldDescriptorProto.Type.TypeBool       , WireType.Variant},
            {FieldDescriptorProto.Type.TypeString     , WireType.String},
            {FieldDescriptorProto.Type.TypeGroup      , WireType.StartGroup}, // TODO and end?
            {FieldDescriptorProto.Type.TypeMessage    , WireType.String},
            {FieldDescriptorProto.Type.TypeBytes      , WireType.String},
            {FieldDescriptorProto.Type.TypeUint32     , WireType.Variant},
            {FieldDescriptorProto.Type.TypeEnum       , WireType.Variant},
            {FieldDescriptorProto.Type.TypeSfixed32   , WireType.Fixed32},
            {FieldDescriptorProto.Type.TypeSfixed64   , WireType.Fixed64},
            {FieldDescriptorProto.Type.TypeSint32     , WireType.Variant},
            {FieldDescriptorProto.Type.TypeSint64     , WireType.Variant},
        };
        static bool IsLengthDelimited(FieldDescriptorProto.Type type)
        {
            return (FieldWireMap[type] == WireType.String);
        }
        static void MakeEncode(GeneratorContext ctx, FieldDescriptorProto obj, string name, string writer, string writer2)
        {
            switch (obj.type)
            {
                case FieldDescriptorProto.Type.TypeDouble:
                    {
                        ctx.WriteLine($@"{writer}.WriteDouble({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeFloat:
                    {
                        ctx.WriteLine($@"{writer}.WriteFloat({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeInt64:
                    {
                        ctx.WriteLine($@"{writer}.WriteInt64({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeUint64:
                    {
                        ctx.WriteLine($@"{writer}.WriteUInt64({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeInt32:
                    {
                        ctx.WriteLine($@"{writer}.WriteInt32({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeFixed64:
                    {
                        ctx.WriteLine($@"{writer}.WriteFixed64({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeFixed32:
                    {
                        ctx.WriteLine($@"{writer}.WriteFixed32({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeBool:
                    {
                        ctx.WriteLine($@"{writer}.WriteBool({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeString:
                    {
                        ctx.WriteLine($@"{writer}.WriteString({name});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeGroup:
                    {
                        bool v = true;
                        if (v) throw new Exception("Unsupported TypeGroup {name}");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeMessage:
                    {
                        ctx.WriteLine(writer + ".WriteMessage((" + writer2 + ") => {");
                        ctx.Indent();
                        ctx.WriteLine($"if({name} != {DefaultValue(obj.type)})");
                        ctx.WriteLine("{");
                        ctx.Indent();
                        ctx.WriteLine($@"{name}.Encode({writer2});");
                        ctx.Outdent();
                        ctx.WriteLine("}");
                        ctx.Outdent();
                        ctx.WriteLine("});");
                ...
                case FieldDescriptorProto.Type.TypeBytes:
                    {
                        ctx.WriteLine($@"{writer}.WriteBytes({name});");
                    }
                    break;
        ...
        static void MakeDecode(GeneratorContext ctx, FieldDescriptorProto obj ,string name,string reader,string reader2)
        {
            switch (obj.type)
            {
                case FieldDescriptorProto.Type.TypeDouble:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadDouble();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeFloat:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadFloat();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeInt64:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadInt64();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeUint64:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadUInt64();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeInt32:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadInt32();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeFixed64:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadUInt32();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeFixed32:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadFixed32();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeBool:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadBool();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeString:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadString();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeGroup:
                    {
                ...
                case FieldDescriptorProto.Type.TypeMessage:
                    {
                        ctx.WriteLine(reader + ".ReadMessage((" + reader2 + ") => {");
                        ctx.Indent();
                        ctx.WriteLine($@"{name} = new {GetTypeName2(obj.TypeName)}();");
                        ctx.WriteLine($@"{name}.Decode({reader2});");
                        ctx.Outdent();
                        ctx.WriteLine("});");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeBytes:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadBytes();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeUint32:
                    {
                ...
                case FieldDescriptorProto.Type.TypeEnum:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadEnum();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeSfixed32:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadSFixed32();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeSfixed64:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadSFixed64();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeSint32:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadSInt32();");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeSint64:
                    {
                        ctx.WriteLine($@"{name} = {reader}.ReadSInt64();");
                    }
                    break;
            }
        }
        static uint MakeTag(int fieldNumber, bool isMap, FieldDescriptorProto.Type fieldType, FieldDescriptorProto.Label labelType)
        {
            return (uint)(fieldNumber << 3) | (uint)((labelType == FieldDescriptorProto.Label.LabelRepeated || isMap) ? WireType.String : FieldWireMap[fieldType]);
        }
        static string DefaultValue(FieldDescriptorProto.Type type)
        {
            string val = "";
            switch (type)
            {
                ...
                case FieldDescriptorProto.Type.TypeGroup:
                    {
                        bool v = true;
                        if (v) throw new Exception($"Unsupported type {type}");
                    }
                    break;
                case FieldDescriptorProto.Type.TypeMessage:
                    {
        ...
        protected override void WriteEncodeEnd(GeneratorContext ctx, DescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            ctx.Outdent();
            ctx.WriteLine("}");
        }
        protected override void WriteDecodeBegin(GeneratorContext ctx, DescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            ctx.WriteLine("public void Decode(Google.Protobuf.CodedInputStream reader)");
            ctx.WriteLine("{");
            ctx.Indent();
        }
        protected override void WriteDecodeDefault(GeneratorContext ctx, FieldDescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            var name = ctx.NameNormalizer.GetName(obj);
            ...
            if (isRepeated)
            {
                ctx.WriteLine($"{ Escape(name)}.Clear();");
            }
            else
            {
                ctx.WriteLine($"{Escape(name)} = " + DefaultValue(obj.type) + ";");
            }

            ctx.WriteLine();
        }
        protected override void WriteDecodeTag(GeneratorContext ctx, DescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            ctx.WriteLine("uint tag;");
            ctx.WriteLine("while ((tag = reader.ReadTag()) != 0)");
            ctx.WriteLine("{");
            ctx.Indent();
        ...
        protected override void WriteDecodeBody(GeneratorContext ctx, FieldDescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            var number = obj.Number;
            var name = ctx.NameNormalizer.GetName(obj);
            ...
            if (isRepeated)
            {
                if (isMap)
                {
                    ...
                    var ktag = MakeTag(1, false, keyType.type, keyType.label);
                    var vtag = MakeTag(2, false, valueType.type, valueType.label);
                    ctx.WriteLine("reader.ReadMapItor((mapr) => {");
                    ctx.Indent();
                    ...
                    if (true)
                    {
                        string _dataFormat;
                        bool _isMap;
                        ctx.WriteLine($"{GetTypeName(ctx, keyType, out _dataFormat, out _isMap)} k = {DefaultValue(keyType.type)};");
                    }
                    if (true)
                    {
                        string _dataFormat;
                        bool _isMap;
                        ctx.WriteLine($"{GetTypeName(ctx, valueType, out _dataFormat, out _isMap)} v = {DefaultValue(valueType.type)};");
                    }
                    ctx.WriteLine($"uint kvtag;");
                    ctx.WriteLine("while((kvtag = mapr.ReadTag()) != 0)");
                    ctx.WriteLine("{");
                    ctx.Indent();
                    ctx.WriteLine("switch(kvtag)");
                    ctx.WriteLine("{");
                    ctx.Indent();
                    ctx.WriteLine($"case {ktag}:");
                    ctx.Indent();
                    ctx.WriteLine("{");
                    ctx.Indent();
                    MakeDecode(ctx, keyType, "k", "mapr", "mapmstr");
                    ctx.Outdent();
                    ctx.WriteLine("}");
                    ctx.WriteLine("break;");
                    ctx.Outdent();
                    ctx.WriteLine($"case {vtag}:");
                    ctx.Indent();
                    ctx.WriteLine("{");
                    ctx.Indent();
                    if (valueType.type == FieldDescriptorProto.Type.TypeMessage)
                    {
                        ctx.WriteLine($@"v = new {GetTypeName2(valueType.TypeName)}();");
                        MakeDecode(ctx, valueType, "v", "mapr", "mapmstr");
                    }
                    else
                    {
                        MakeDecode(ctx, valueType, "v", "mapr", "mapmstr");
                    ...
                    ctx.WriteLine("{");
                    ctx.Indent();
                    ctx.WriteLine("mapr.SkipLastField();");
                    ctx.Outdent();
                    ctx.WriteLine("}");
                    ctx.WriteLine("break;");
                    ctx.Outdent();
                    ...
                    ctx.Outdent();
                    ctx.WriteLine("}");
                    ctx.WriteLine($"{Escape(name)}.Add(k,v);");
                    ctx.Outdent();
                    ctx.WriteLine("});");
                }
                else
                {
                    if (IsLengthDelimited(obj.type))
                    {
                        if (obj.type == FieldDescriptorProto.Type.TypeMessage)
                        {
                            ctx.WriteLine($@"{GetTypeName2(obj.TypeName)} o = {DefaultValue(obj.type)};");
                            MakeDecode(ctx, obj, "o", "reader", "msgr");
                            ctx.WriteLine($@"{Escape(name)}.Add(o);");
                        }
                        else
                        {
                            MakeDecode(ctx, obj, "var o", "reader", "msgr");
                            ctx.WriteLine($@"{Escape(name)}.Add(o);");
                        }
                    }
                    else
                    {
                        ctx.WriteLine("reader.ReadRepeated((listr) => {");
                        ctx.Indent();
                        if (obj.type == FieldDescriptorProto.Type.TypeMessage)
                        {
                            ctx.WriteLine($@"{GetTypeName2(obj.TypeName)} o = {DefaultValue(obj.type)};");
                            MakeDecode(ctx, obj, "o", "listr", "listmsgr");
                            ctx.WriteLine($@"{Escape(name)}.Add(o);");
                        }
                        else
                        {
                            MakeDecode(ctx, obj, "var o", "listr", "listmsgr");
                            ctx.WriteLine($@"{Escape(name)}.Add(o);");
                        }
                        ctx.Outdent();
                        ctx.WriteLine("});");
            ...
            {
                MakeDecode(ctx, obj, Escape(name), "reader", "msgr");
            }
            ctx.Outdent();
            ctx.WriteLine("}");
            ctx.WriteLine("break;");
        ...
        protected override void WriteDecodeEnd(GeneratorContext ctx, DescriptorProto obj, ref object state, OneOfStub[] oneOfs)
        {
            ctx.WriteLine("default:");
            ctx.Indent();
            ctx.WriteLine("{");
            ctx.Indent();
            ctx.WriteLine("reader.SkipLastField();");
            ctx.Outdent();
            ctx.WriteLine("}");
            ctx.WriteLine("break;");
            ctx.Outdent();
